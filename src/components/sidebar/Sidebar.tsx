...
import { ChevronDown, ChevronRight, Eye, Filter, Info, LayoutGrid, PanelLeftClose, PanelLeft, Repeat2, Search, Zap } from 'lucide-react'
...
export default function Sidebar() {
  ...
  const noBlocks = useMemo(() => {
    const tg = getBlocksByCategoryGroupedByService('trigger')
    const ac = getBlocksByCategoryGroupedByService('action')
    const fl = getBlocksByCategoryGroupedByService('filter')
    const dp = getBlocksByCategoryGroupedByService('display')
    const hasTriggers = filterBlocks([...tg.general, ...Object.values(tg.byService).flat()], query).length > 0
    const hasActions = filterBlocks([...ac.general, ...Object.values(ac.byService).flat()], query).length > 0
    const hasFilters = filterBlocks([...fl.general, ...Object.values(fl.byService).flat()], query).length > 0
    const hasDisplay = filterBlocks([...dp.general, ...Object.values(dp.byService).flat()], query).length > 0
